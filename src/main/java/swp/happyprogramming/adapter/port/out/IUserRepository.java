...
public interface IUserRepository extends JpaRepository<User, Long> ,
  ...
  @Query(value = "select * from users where id in "
    + "(select user1 from connections where user2 = ?1 union select user2 from connections where user1 = ?1)", nativeQuery = true)
  ...
  @Query(value = "select * from users where id in (select mentor_id from request where mentee_id = (select id from users where email=?1) union select mentee_id from request where mentor_id = (select id from users where email=?1))", nativeQuery = true)
  ...
  @Query(value =
    "SELECT sum(CASE WHEN b.created IS NULL THEN 0 ELSE 1 END ) FROM \n" + "(\n"
